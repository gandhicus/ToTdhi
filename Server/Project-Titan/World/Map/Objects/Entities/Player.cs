...
namespace World.Map.Objects.Entities
{
    public partial class Player : Entity
    {
        ...
        private bool IsValidSkin(ushort skin)
        {
            if (skin == 0) return true;
            if (!GameData.objects.TryGetValue(skin, out var skinObject)) return false;
            if (!(skinObject is SkinUnlockerInfo skinInfo)) return false;
            return skinInfo.characterType == info.id;
        }
