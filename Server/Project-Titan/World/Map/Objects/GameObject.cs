...
namespace World.Map.Objects
{
    ...
    {
        private const float Min_Object_Size = 0.05f;
        private const float Max_Object_Size = 4f;
        ...
        public virtual void Initialize(GameObjectInfo info)
        {
            ...
            var initialSize = SanitizeSize(info.size.min);
            size.SetDefault(initialSize);
        ...
        {
            this.size.Value = SanitizeSize(size);
        }
        ...
        private float SanitizeSize(float size)
        {
            if (float.IsNaN(size) || float.IsInfinity(size))
                size = info?.size.min ?? 1f;

            return Math.Max(Min_Object_Size, Math.Min(Max_Object_Size, size));
        }
