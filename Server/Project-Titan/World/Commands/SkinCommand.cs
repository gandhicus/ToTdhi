...
namespace World.Commands
{
    public class SkinCommand : CommandHandler
    {
        ...
        public override ChatData Handle(Player player, CommandArgs args)
        {
            ...
            if (!(info is SkinUnlockerInfo skinInfo))
                return ChatData.Error($"'{info.name}' is not a player skin.");

            if (skinInfo.characterType != player.info.id)
            {
                var correctClass = GameData.objects[(ushort)skinInfo.characterType];
                return ChatData.Error($"'{info.name}' is only available for the '{correctClass.name}' class.");
            }

            player.SetSkin(skinInfo.id);
