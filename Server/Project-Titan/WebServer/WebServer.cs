...
namespace WebServer
{
    public class WebServer
    {
        ...
        public WebServer()
        {
            ...
            listener.AddHandler("v1/purchase/local/free", HandleLocalFreePurchase);
        ...
        private async Task<object> HandleServerList(HttpListenerContext context, NameValueCollection query)
        {
            ...
            return new WebServerListResponse(WebServerListResult.Success, serverList.GetInfos(context.Request.RemoteEndPoint.Address));
        }
        ...
        private async Task<object> HandleLocalFreePurchase(HttpListenerContext context, NameValueCollection query)
        {
            if (!rateLimiter.CanRequest(context.Request.RemoteEndPoint.Address))
            {
                return new WebVerifyResponse(false);
            ...
            if (AnyNullOrEmpty(token, productId)) return new WebVerifyResponse(false);

            token = DecryptString(token);

            var tokenResponse = await TokenLogin.Get(token);
            if (tokenResponse.result != Model.RequestResult.Success) return new WebVerifyResponse(false);
            if (!IapProduct.idToProducts.TryGetValue(productId, out var product)) return new WebVerifyResponse(false);
            if (product.currencyReward <= 0) return new WebVerifyResponse(false);

            return new WebVerifyResponse(await DispatchGold(tokenResponse.item.accountId, product.currencyReward));
        }
