    ...
    where T : IChunk
{
    ...
    protected void ApplySprite(int localX, int localY, Sprite sprite, TileRotation rotation = TileRotation.None)
    {
        ...
        ApplyRotatedUvRect(uvs, index, rect, textureSize, rotation);
    ...
    private static void ApplyRotatedUvRect(Vector2[] target, int index, UnityEngine.Rect rect, Vector2 textureSize, TileRotation rotation)
    {
        ...
        target[index + i] = new Vector2(rect.xMin, rect.yMin) / textureSize;
        i = (i + 1) % 4;

        target[index + i] = new Vector2(rect.xMax, rect.yMin) / textureSize;
        i = (i + 1) % 4;

        target[index + i] = new Vector2(rect.xMax, rect.yMax) / textureSize;
        i = (i + 1) % 4;

        target[index + i] = new Vector2(rect.xMin, rect.yMax) / textureSize;
    }
